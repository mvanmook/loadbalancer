...
namespace Loadbalancer{
  const char *stateLookup[] ={"Offline",
                               "Starting monitoring",
                               "Monitored (error)",
                               "Monitored (not in service)",
                               "Requesting stop",
                               "Requesting clean",
  ...
  hostDetails::hostDetails(char *name)
      : hostMutex(0), name(name), ramCurr(0), ramMax(0), availBandwidth(128 * 1024 * 1024),
        addBandwidth(0), balanceP(0), balanceRedirect(""){}
  /**
   * destructor
   */
   ...
   */
  void hostDetails::update(JSON::Value fillStateOut, JSON::Value fillStreamsOut,
                           uint64_t scoreSource, uint64_t scoreRate, uint64_t toAdd){
    if (!hostMutex){hostMutex = new tthread::recursive_mutex();}
    tthread::lock_guard<tthread::recursive_mutex> guard(*hostMutex);
  ...
  void hostDetails::update(JSON::Value fillStateOut, JSON::Value fillStreamsOut, uint64_t scoreSource,
    ...
    for (int i = 0; i < streams.size(); i++){
      s.insert(std::pair<std::string, streamDetails>(
          streams[STREAMKEY][i]["key"], *(x.destringify(streams[STREAMKEY][i]["streamDetails"]))));
    }
    update(fillStateOut, fillStreamsOut, scoreSource, scoreRate, out,
           conf_streams.asSet<std::string>(), s, tags.asSet<std::string>(), cpu, servLati,
           servLongi, binHost, host, toAdd, currBandwidth, availBandwidth, currRam, ramMax);
  }
  ...
  void hostDetailsCalc::calc(){
    ...
    j[STREAMKEY] = convertMapToJson(streams);
    j[TAGKEY] = tags;
    ...
    j["currBandwidth"] = upSpeed + downSpeed;
    j["availBandwidth"] = availBandwidth;
    j["currram"] = ramCurr;
    j["ramMax"] = ramMax;
  ...
  void hostDetailsCalc::update(JSON::Value &d){
    ...
    if (d.isMember(STREAMKEY) && d[STREAMKEY].size()){
      jsonForEach(d[STREAMKEY], it){
      ...
      if (streams.size()){
        ...
        for (std::map<std::string, streamDetails>::iterator it = streams.begin(); it != streams.end(); ++it){
          if (!d[STREAMKEY].isMember(it->first)){eraseList.insert(it->first);}
        }
  ...
  std::string geohash(double lat, double lng, int precision){
